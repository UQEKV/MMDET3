...
def show_multi_modality_result(img,
                               ...
                               labels=None, scores=None):    #adding labels to 3d bounding boxes
    ...
    if show:
        ...
        # show_img = show_img.resize([375, 1242, 3])
        if gt_bboxes is not None:
            show_img = draw_bbox(
                gt_bboxes, show_img, proj_mat, img_metas, color=gt_bbox_color)
        if pred_bboxes.tensor.cpu().numpy().shape[0] != 0:
            # print( pred_bboxes.tensor.cpu().numpy().shape[0])
            show_img = draw_bbox(
                pred_bboxes,
                show_img,
                proj_mat,
                img_metas,
                color=pred_bbox_color, labels_3d=labels, scores_3d=scores)
        ...
        print('Saving in path: ',osp.join(result_path, f'{filename}.png'))
