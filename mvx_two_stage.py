    ...
    def show_results(self, data, result, out_dir):
        ...
        for batch_id in range(len(result)):
            ...
            if isinstance(data['img_metas'][0], DC):
                # img_metas = data['img_metas'][0]._data[0][batch_id]
                pts_filename = data['img_metas'][0]._data[0][batch_id][
                    'pts_filename']
                box_mode_3d = data['img_metas'][0]._data[0][batch_id][
                    'box_mode_3d']
            elif mmcv.is_list_of(data['img_metas'][0], dict):
                # img_metas = data['img_metas'][0][batch_id]
            ...
            inds = result[batch_id]['pts_bbox']['scores_3d'] > 0.4   #using confidence score to filter the 3d boundingboxes
            pred_bboxes = result[batch_id]['pts_bbox']['boxes_3d'][inds]
            labels_3d = result[batch_id]['pts_bbox']['labels_3d'][inds]
            scores_3d = result[batch_id]['pts_bbox']['scores_3d'][inds]
            # print(result)

            # print(range(len(result)))
            # print(result[batch_id])
            # for now we convert points and bbox into depth mode / lidar mode
            ...
            # print(file_name)
            # imgs = data['img'][0]._data[0][batch_id].cpu().numpy().transpose((1,2,0))
            img_path = './data/kitti/testing/image_2/'
            # print(osp.join(img_path, f'{file_name}.png'))

            # read from file because img in data_dict has undergone pipeline transform
            imgs = mmcv.imread(osp.join(img_path, f'{file_name}.png'))
            # print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.')
